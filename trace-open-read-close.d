...
/self->read_time/
{
	...
	printf("[%Y] %10s: %-20s %-50s [pid %d | fd %d] (%d ns / %d us / %d ms)\n",
	    walltimestamp, execname, this->title, self->read_filename, pid,
	    self->read_fd_num, this->delta_ns, this->delta_us, this->delta_ms);
...
/my_time[pid, arg0]/
{
	...
	printf("[%Y] %10s: %-20s %-50s [pid %d | fd %d]\n", walltimestamp,
	    execname, this->title, self->filename, pid,
	    self->new_fd_num);
...
/self->delta/
{
	...
	printf("[%Y] %10s: %-20s %-50s [pid %d | fd %d] (%d ns / %d us / %dms) Duration (%d ns / %d us / %d ms)\n",
	    walltimestamp, execname, this->title, self->filename, pid,
	    self->new_fd_num, this->delta_ns, this->delta_us,
	    this->delta_ms, this->duration_delta_ns,
	    this->duration_delta_us, this->duration_delta_ms);
